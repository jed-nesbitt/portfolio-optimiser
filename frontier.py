...
def generate_frontier_cloud(mu, cov, rf, num_portfolios, seed, max_weight):
    ...
    if max_weight < 1.0 / n:
        raise ValueError(f"max_weight={max_weight} too small for n={n}. Must be >= {1.0/n:.4f}")
...
def pick_min_vol_for_target_return(frontier_df, weights_array, target_return, tolerance=0.002):
...
# SCIPY OPTIMIZATION
# -----------------------------
def optimize_max_sharpe(mu, cov, rf, bounds):
    n = len(mu)
...
def optimize_min_vol(mu, cov, bounds):
    n = len(mu)
    ...
    cons = {"type": "eq", "fun": lambda w: np.sum(w) - 1}
    w0 = np.ones(n) / n
    res = minimize(vol, w0, bounds=bounds, constraints=cons)
    return res.x / np.sum(res.x)


def optimize_target_return(mu, cov, target, bounds):
    n = len(mu)
...
# FRONTIER RUNNER
# -----------------------------
def save_frontier_plot(frontier_df, out_path: Path, special_points: Dict[str, Tuple[float, float]]):
    import matplotlib.pyplot as plt

    out_path.parent.mkdir(parents=True, exist_ok=True)
    plt.figure(figsize=(10, 7))
    plt.scatter(frontier_df["Volatility"], frontier_df["ExpectedReturn"],
                c=frontier_df["Sharpe"], s=10, alpha=0.6)
    ...
    plt.title("Efficient Frontier (Random + SciPy Portfolios)")
    plt.legend()
    plt.grid(True)
...
def run_frontier(
    ...
    # --- Random Frontier ---
    frontier_df, weights_array = generate_frontier_cloud(mu, cov, rf, num_portfolios, seed, max_weight)
    ...
    print("Selected random frontier portfolios (non-SciPy).")

    if target_return is not None:
        w_target = pick_min_vol_for_target_return(frontier_df, weights_array, target_return, tolerance=target_tolerance)
        optimals[f"TargetReturn_{target_return:.2%}"] = w_target
        print(f"Added random TargetReturn_{target_return:.2%}")

    # --- SciPy Optimization ---
    bounds = [(0.0, max_weight)] * len(mu)
    optimals["SciPy_MaxSharpe"] = optimize_max_sharpe(mu, cov, rf, bounds)
    optimals["SciPy_MinVol"] = optimize_min_vol(mu, cov, bounds)

    if target_return is not None:
        optimals[f"SciPy_TargetReturn_{target_return:.2%}"] = optimize_target_return(mu, cov, target_return, bounds)

    print("✅ Added SciPy portfolios (MaxSharpe, MinVol, and TargetReturn).")
